...
installbbr(){
	...
	detele_kernel
	BBR_grub
	echo -e "${Tip} After restarting the VPS, please re-run the script to enable ${Red_font_prefix}BBR/BBRplus${Font_color_suffix}"
	stty erase '^H' && read -p "You need to restart the VPS before you can start the BBR/BBRplus. Do you want to restart now? [Y/n] : " yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
...
installbbrplus(){
	...
	stty erase '^H' && read -p "The VPS needs to be restarted before BBRplus can be started. Do you want to restart now?[Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
		echo -e "${Info} VPS restarting..."
		reboot
...
installlot(){
	...
	stty erase '^H' && read -p "VPS needs to be restarted before Lotserver can be started. Do you want to restart now?[Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
		echo -e "${Info} VPS restarting..."
		reboot
...
startbbrmod(){
	...
	sysctl -p
    cd .. && rm -rf bbrmod
	echo -e "${Info}BBRmod started successfully！"
}
...
remove_all(){
  ...
  sed -i '/fs.file-max/d' /etc/sysctl.conf
	...
	clear
	echo -e "${Info}: removed all successfully"
	sleep 1s
}
...
*               hard    nofile          1000000">/etc/security/limits.conf
	...
	read -p "You need to restart the VPS before the system optimization configuration will take effect. Do you want to restart now?[Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
		echo -e "${Info} VPS restarting..."
		reboot
...
#更新脚本
Update_Shell(){
	echo -e "current version is [ ${sh_ver} ]，checking for latest version..."
...
echo && echo -e " TCP accelerated one-click installation management script ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
 ...
 ${Green_font_prefix}0.${Font_color_suffix} upgrade
————————————内核管理————————————
 ${Green_font_prefix}1.${Font_color_suffix} install BBR/BBRmod kernel
...
————————————加速管理————————————
 ...
 ${Green_font_prefix}5.${Font_color_suffix} use BBRmod to accelerate
 ${Green_font_prefix}6.${Font_color_suffix} use violentBBRmod(does not support some systems)
 ${Green_font_prefix}7.${Font_color_suffix} use BBRplus to accelerate
 ${Green_font_prefix}8.${Font_color_suffix} use Lotserver(sharp speed) accelerate
————————————杂项管理————————————
 ${Green_font_prefix}9.${Font_color_suffix} uninstall all
...
————————————————————————————————" && echo
	...
	if [[ ${kernel_status} == "noinstall" ]]; then
		echo -e " current status: ${Green_font_prefix} is not installed ${Font_color_suffix} accelerated kernel ${Red_font_prefix} please install the kernel ${Font_color_suffix}"
	else
...
case "$num" in
	...
	*)
	clear
	echo -e "${Error}:please enter a correct number [0-11]"
	sleep 5s
	start_menu
	;;
esac
	...
	if [[ "${release}" == "centos" ]]; then
		...
		if [ "${rpm_total}" > "1" ]; then
			...
			for((integer = 1; integer <= ${rpm_total}; integer++)); do
				...
				echo -e "start unloading ${rpm_del} kernel..."
				rpm --nodeps -e ${rpm_del}
				echo -e "Uninstall ${rpm_del} kernel uninstall complete, continue..."
			done
			echo --nodeps -e "The kernel has been unloaded, continue..."
		else
		...
		if [ "${deb_total}" > "1" ]; then
			echo -e "detected ${deb_total} remaining cores , starting to uninstall..."
	...
	if [[ ${kernel_status} == "Lotserver" ]]; then
		if [[ -e /appex/bin/lotServer.sh ]]; then
			...
			if [[ ${run_status} = "running!" ]]; then
				run_status="Lotserver started successfully"
			else 
				run_status="Lotserver failed to start"
			fi
		else 
		...
		if [[ ${run_status} == "bbr" ]]; then
			...
			if [[ ${run_status} == "tcp_tsunami" ]]; then
				run_status="BBRmod started successfully"
			else 
				run_status="BBRmod failed to start"
			fi
		elif [[ ${run_status} == "nanqinlang" ]]; then
			run_status=`lsmod | grep "nanqinlang" | awk '{print $1}'`
			if [[ ${run_status} == "tcp_nanqinlang" ]]; then
				run_status="violentBBRmod started successfully"
			else 
				run_status="violentBBRmod failed to start"
			fi
		else 
			run_status="Acceleration module not installed"
		fi
